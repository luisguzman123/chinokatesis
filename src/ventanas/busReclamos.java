...
public class busReclamos extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        grillaBuscador.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Código", "Obs", "Fecha", "Estado", "Cod Servicio terminado", "Desc Servicio", "Cod cliente", "Cliente", "Cod Empleado", "Empleado"
            }
        ) {
    ...
    private void seleccionarReclamos() {

        int fila = grillaBuscador.getSelectedRow();

        String cod = grillaBuscador.getValueAt(fila, 0).toString();
        String obs = grillaBuscador.getValueAt(fila, 1).toString();
        String fecha = grillaBuscador.getValueAt(fila, 2).toString();
        String estado = grillaBuscador.getValueAt(fila, 3).toString();
        String codServis = grillaBuscador.getValueAt(fila, 4).toString();
        String descServis = grillaBuscador.getValueAt(fila, 5).toString();
        ...
        if (busqueda.equals("reclamos")) {   //primero se realiza esta accion porque de otro modo vacia la variable "operacion"
            reclamos.txtCod.setText(cod);
            reclamos.txtServicioTerminado.setText(codServis);
            reclamos.txtDescriServicioTermi.setText(descServis);
            ...
            reclamos.txtObs.setText(obs);
            reclamos.txtCod.requestFocus();
            busqueda = "";
...
//            Metodos.limpiarTabla(recepcion.grilla);
//            if (detalles.isBeforeFirst()) {
//                while (detalles.next()) {
//                    Metodos.cargarTabla(recepcion.grilla, new Object[]{
...
//            Logger.getLogger(busReclamos.class.getName()).log(Level.SEVERE, null, ex);
...
//        }
   ...
   public void getDatos() {
    ...
    private void buscarReclamos() {
        ...
        try {
            cn.conectar();
            ResultSet recep = cn.consultar("select * from v_reclamos where fecha_entada BETWEEN '" + Metodos.dameFechaFormateadaSQL(DESDE.getDate()) + "' and '"+Metodos.dameFechaFormateadaSQL(HASTA.getDate())+"' and estado !='ANULADO'  order by id_reclamo_cab"); //order by ordena de menor a mayor, si se quiere de mayor a menor se le agrega desc al final
            Metodos.limpiarTabla(grillaBuscador);
            if (recep.isBeforeFirst()) {
                while (recep.next()) {
                    
                    Metodos.cargarTabla(grillaBuscador, new Object[]{recep.getString("id_reclamo_cab"), recep.getString("obs"),recep.getString("fecha_entada"), recep.getString("estado"),recep.getString("id_servi_termi_cab"), recep.getString("descri"),recep.getString("id_cliente"), recep.getString("cliente"), recep.getString("id_empleado"), recep.getString("empleado")});
                }
            } else {
