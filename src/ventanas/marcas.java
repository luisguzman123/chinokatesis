...
public class marcas extends javax.swing.JFrame {
    ...
    private void btnAgregarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAgregarActionPerformed
        ...
        txtCodigo.setText(Metodos.ultimoCodigo("id_marca", "marcas"));
    ...
    private void btnGrabarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGrabarActionPerformed
        int respuesta = 5;
        duplicado = Metodos.evitarDuplicado("marcas", "mar_desc", txtNombre.getText().trim(), "id_marca", txtCodigo.getText());
        ...
        if (respuesta==0) {
            ...
            if(operacion.equals("agregar")){
                sql="Insert into marcas (mar_desc) "
                + "values ('"+txtNombre.getText().toUpperCase().trim()+"')";
            }
            
            if(operacion.equals("modificar")){
                sql="update marcas set mar_desc = '"+txtNombre.getText().toUpperCase().trim()+"'  where id_marca = "+txtCodigo.getText();
            }
            
            if(operacion.equals("borrar")){
                sql="Delete from marcas where id_marca = "+txtCodigo.getText();
            }
...
//       }
      ...
      if(busqueda.equals("equipos") && operacion.equals("")){   //primero se realiza esta accion porque de otro modo vacia la variable "operacion"
           equipos.txtMarca.setText(cod+"-"+nombre);
           equipos.txtModelo.setEditable(true);
           equipos.txtModelo.requestFocus();
           busqueda = "";
           dispose();
       }
    ...
    public void getDatos(){
        ...
        try {
            cn.conectar();
            ResultSet provee=cn.consultar("select * from marcas where mar_desc ilike '%"+txtBuscador.getText()+"%' order by id_marca"); //order by ordena de menor a mayor, si se quiere de mayor a menor se le agrega desc al final
            Metodos.limpiarTabla(grillaBanco);
            if(provee.isBeforeFirst()){
                while(provee.next()){
                    Metodos.cargarTabla(grillaBanco, new Object[]{provee.getString("id_marca"), provee.getString("mar_desc")});
                }
            }else{
